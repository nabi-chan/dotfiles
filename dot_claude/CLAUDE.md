...
- 독립적으로 병렬 수행 가능한 서브태스크가 2개 이상일 때 팀 사용
- 단순 순차 작업이나 서로 의존적인 작업은 팀 없이 직접 수행

## 싱크탱크 (Obsidian vault)

- 경로: `~/nabi/obsidian/`
- 프로젝트 로그: `10-프로젝트/<프로젝트명>/프로젝트 노트/claude-log.md`에 append
- 해당 프로젝트 폴더가 없으면 사용자에게 생성 여부 확인 후 진행
- 새 대화 시작 시 관련 프로젝트 로그가 있으면 먼저 읽어서 맥락 파악
- 기록 시점:
  - 중요한 아키텍처/기술 결정을 내렸을 때
  - 삽질 끝에 해결한 문제와 원인
  - 프로젝트 마일스톤 완료 시
  - 나중에 참고할 만한 패턴이나 인사이트
- 기록 형식:
  ```
  ### YYYY-MM-DD
  - 작업 내용 요약
  - 주요 결정사항과 이유
  ```
- 사소한 코드 변경이나 단순 버그 수정은 기록하지 않는다

## Temp Files

- 임시 파일은 `~/.claude-temp/<목적>` 하위에 폴더로 관리
